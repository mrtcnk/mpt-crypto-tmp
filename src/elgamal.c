        ...
        unsigned char const* blinding_factor)
{
    ...
    /* This checks the "ephemeral" part of the ElGamal encryption */
    ...
    /* Calculate the Shared Secret S = k * Q (the "mask" for the amount) */
    s_shared = *pubkey_Q;
    ...
    if (amount == 0)
    {
        /* Mathematically, 0 * G is the point at infinity.
           In addition, Infinity + S = S. */
        expected_c2 = s_shared;
    }
    else
    {
        /* Convert uint64_t to 32-byte big-endian scalar */
        for (int i = 0; i < 8; ++i)
        {
            amount_scalar[31 - i] = (amount >> (i * 8)) & 0xFF;
        }
    ...
    return 1;  /* Success: The encryption is correct */
}